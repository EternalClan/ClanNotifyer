...
class Create {
	...
	static table(database, tableName, tableID) {
		const dataTable = database.prepare(`SELECT count(*) FROM sqlite_master WHERE type='table' AND name = '${table}';`).get();
		if (!dataTable["count(*)"]) {
	...
	static tableWithColums(database, table, id, columns) {
		// setTimeout(function () {
			let sql_DB = "";
			if (database === "Config") sql_DB = SQL.config();
			if (database === "Toggle") sql_DB = SQL.toggle();
			if (database === "ChannelRole") sql_DB = SQL.channelRole();
			if (database === "Notifyer") sql_DB = SQL.notifyer();
			if (sql_DB === "") return;
			const dataTable = sql_DB.prepare(`SELECT count(*) FROM sqlite_master WHERE type='table' AND name = '${table}';`).get();
			if (!dataTable["count(*)"]) {
				// If the table isn't there, create it and setup the database correctly.
				const allColumns = `${id} VARCHAR PRIMARY KEY, ${columns}`;
				sql_DB.prepare(`CREATE TABLE ${table} (${allColumns});`).run();
				// Ensure that the "id" row is always unique and indexed.
				sql_DB.prepare(`CREATE UNIQUE INDEX idx_${table}_id ON ${table} (${id});`).run();
				sql_DB.pragma("synchronous = 1");
				sql_DB.pragma("journal_mode = wal");
			} else if (dataTable["count(*)"]) {
				// require(`./columns/${database}/${table}`)();
				if (database !== "") this.#privatTableColumns(sql_DB, table, columns);
				if (database === "") return;
			}
		// }, 500);
	}
