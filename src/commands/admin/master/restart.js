...
module.exports = {
	...
	async execute(interaction) {
		if (interaction == null
		|| interaction.channel.id == null) return console.log(`[${DateTime.utc().toFormat(timeFormat)}][ClanBot] Interaction of Command 'restart' returned 'null / undefined'.`);
		...
		if (dataCommandAdmin.Restart !== "true") return await interaction.reply({ content: langError.command.disabled, ephemeral: true });
		...
		await interaction.reply({ content: `${lang.cmd.admin.restart.nope}`, ephemeral: true });
	}
};